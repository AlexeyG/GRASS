...
DataStore::LinkMap::const_iterator DataStore::AddLink(int groupId, const ContigLink &link)
{
	if (groupId >= GroupCount)
		throw exception();
	LinkCount++;
	///fprintf(stderr, "Linked: d(%i,%i) = %8.2f; orientation: %8s; order: %7s.\n", link.First, link.Second, link.Mean, (link.EqualOrientation ? "equal" : "opposite"), (link.ForwardOrderA ? "forward" : "reverse"));
...
// remove nested loop, support extractig banned links
void DataStore::Extract(const vector<int> &what, DataStore &store)
{
	vector<int> transContig(ContigCount, -1);
	vector<int> transGroup(GroupCount, -1);
	int nWhat = what.size();
	for (int i = 0; i < nWhat; i++)
		transContig[what[i]] = store.AddContig((*this)[what[i]]);
	for (int i = 0; i < nWhat; i++)

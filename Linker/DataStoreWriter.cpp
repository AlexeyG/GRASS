	...
	for (int i = 0; i < nContigs; i++)
		for (int j = 0; j < nContigs; j++)
		{
			...
			for (DataStore::LinkMap::const_iterator it = range.first; it != range.second; it++)
				fprintf(out, "%i\t%i\t%i\t%i\t%i\t%lf\t%lf\t%i\t%lf\n", it->second.GetGroupID(), it->first.first, it->first.second, (it->second.EqualOrientation ? 1 : 0), (it->second.ForwardOrder ? 1 : 0), it->second.Mean, it->second.Std, (it->second.Ambiguous ? 1 : 0), it->second.Weight);
		}
	return true;
}
